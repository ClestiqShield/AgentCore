...
class ToonConverter:
    ...
    def convert_with_metrics(cls, data: Any) -> Tuple[str, int, Dict[str, Any]]:
        ...
        compression_ratio = (
            (1 - (toon_chars / original_chars)) * 100 if original_chars > 0 else 0
        )
...
async def toon_conversion_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Convert to TOON
    toon_str, tokens_saved, conversion_metrics = ToonConverter.convert_with_metrics(
        clean_input
    )
